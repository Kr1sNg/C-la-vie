...
passed as parameter. What we call a "line that has been read" is a succession of
0 to n characters that end with '\n' (ascii code 0x0a) or with End Of File (EOF).
...
be free-able. Your program will be compiled with the flag -D BUFFER_SIZE=xx,
...
function should keep 0 memory allocated with malloc, except the line that has
been returned.
...
of get_next_line(). Finally we consider that get_next_line() has an undefined
behaviour when reading from a binary file.
